...
import { formatMonthDay, getKoreanDayOfWeek, padNumber, formatTimeDifference, formatSpawnTime, calculateNearestFutureTime, formatToTimestamp } from './utils.js';
...
export function updateNextBossDisplay(DOM) {
    ...
    if (nextBoss) {
       ...
       const remainingTimeString = `${formatToTimestamp(nextBoss.timestamp)} 젠`; // Remove parentheses
...
export function renderUpcomingBossList(DOM) {
    ...
    if (upcomingBosses.length > 0) {
        ...
        upcomingBosses.slice(1).forEach(boss => {
            ...
            const remaining = `(${formatToTimestamp(boss.timestamp)})`;
...
export function renderUpdateModal(DOM, noticeData) {
    ...
    const version = window.APP_VERSION || "2.17.0";
